...
def sort_list(shows, totalPages, showTime):
...
@app.route('/weekly/<int:page>/', methods=['GET', 'POST'])
...
def show_info():
	...
	shows, totalPages = getAirsToday(1)
	showsTwo, totalPages = getAirsToday(2)
	USshows = sort_list(shows, totalPages, daily)
	USshows_two = sort_list(showsTwo, totalPages, daily)
	for show in USshows_two:
		USshows.append(show)
	#USshows.append(USshows_two)
	flask.flash(totalPages)
	...
	return flask.render_template('testing.html', html_class='blah', shows=USshows)
