...
def sort_list(shows):
	#return shows[0]['origin_country']
	new_data = []
	new_thing = []
	for show in shows:
		new_data.append(show['origin_country'])

	# num = 0
	# for i in range(0,len(shows)):
	# 	new_data.append(shows[i]['origin_country'])

	# for i in range(0, len(shows)):
	# 	if shows[i]['origin_country'] == 'US':
	# 		num += 1
	#return filter(lambda entry: entry['origin_country'][0] == 'US', shows)
	# new_data = []
	# thing = 0
	# for entry in data:
	# 	country = entry['origin_country']
	# 	new_data.append(country)
	# 	# if entry['origin_country'] == 'US':
	# 	# 	thing +=1
...
def show_info():
	...
	return flask.render_template('testing.html', html_class='blah', shows=shows)
